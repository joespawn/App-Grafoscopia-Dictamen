...
const DocumentoscopyView: React.FC<DocumentoscopyViewProps> = ({ data }) => {
  ...
  // const expertName = data.expertName || "[NOMBRE DEL PERITO]"; // Not used in specific text provided but available
      ...
      case 'A':
        return (
          <div className="mt-8 glass-card bg-green-50/50 p-6 rounded-2xl border-green-200/50">
            ...
            <p className="font-bold text-green-800 text-sm mb-2">Opción 1: Correspondencia de Autenticidad (Documento Íntegro)</p>
      ...
      case 'B':
        return (
          <div className="mt-8 glass-card bg-red-50/50 p-6 rounded-2xl border-red-200/50">
             ...
             <p className="font-bold text-red-800 text-sm mb-2">Opción 2: Determinación de Falsedad (Documento Alterado o Inauténtico)</p>
      ...
      <div className="max-w-6xl mx-auto w-full p-8 space-y-8 min-h-full">
        ...
        <div className="glass-panel p-8 rounded-3xl">
          ...
          <div className="mt-8">
            ...
            <div className="prose prose-slate max-w-none text-justify text-slate-800 leading-relaxed space-y-4 font-serif text-lg">
              <p>
              ...
              <h4 className="font-bold text-xl text-slate-900 mt-6">1. Elementos Constitutivos del Documento</h4>
              ...
              <h4 className="font-bold text-xl text-slate-900 mt-6">2. La Técnica: ¿Cómo se analizan las alteraciones?</h4>
              ...
              <p className="mt-4 font-medium italic bg-slate-50 p-4 border-l-4 border-slate-400">
            ...
            <div className="space-y-4">
              <h3 className="text-xl font-bold text-red-700 uppercase border-b border-red-200/50 pb-2">
                DOCUMENTO CUESTIONADO
              </h3>
              
              <div className="glass-card bg-white/50 p-6 rounded-xl border-2 border-dashed border-red-200/50">
                <div className="mb-4">
                  <label className="cursor-pointer inline-flex items-center px-4 py-2 bg-white/50 border border-slate-300/50 rounded-lg font-semibold text-slate-700 uppercase tracking-widest hover:bg-slate-50/50 active:bg-slate-100/50 transition ease-in-out duration-150 shadow-sm backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {questionedImages.map((img, idx) => (
                      <div key={idx} className="relative group rounded-xl overflow-hidden glass-card p-2">
                        <img src={img} alt={`Cuestionado ${idx + 1}`} className="w-full h-64 object-contain rounded-lg" />
                        <button 
                        ...
                        <div className="p-2 text-center text-xs text-slate-500 font-mono font-bold">FIGURA DOC-C-{idx + 1}</div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-center text-slate-400 py-8 italic border border-slate-200/50 rounded-lg">
            ...
            <div className="space-y-4 mt-8">
              <h3 className="text-xl font-bold text-green-700 uppercase border-b border-green-200/50 pb-2">
                DOCUMENTO INDUBITABLE
              </h3>
              
              <div className="glass-card bg-white/50 p-6 rounded-xl border-2 border-dashed border-green-200/50">
                <div className="mb-4">
                  <label className="cursor-pointer inline-flex items-center px-4 py-2 bg-white/50 border border-slate-300/50 rounded-lg font-semibold text-slate-700 uppercase tracking-widest hover:bg-slate-50/50 active:bg-slate-100/50 transition ease-in-out duration-150 shadow-sm backdrop-blur-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
                ...
                {undisputedImages.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {undisputedImages.map((img, idx) => (
                      <div key={idx} className="relative group rounded-xl overflow-hidden glass-card p-2">
                        <img src={img} alt={`Indubitable ${idx + 1}`} className="w-full h-64 object-contain rounded-lg" />
                        <button 
                        ...
                        <div className="p-2 text-center text-xs text-slate-500 font-mono font-bold">FIGURA DOC-I-{idx + 1}</div>
                      </div>
                    ))}
                  </div>
                ) : (
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-slate-600">
                Describa de forma manual lo que se aprecia en cada dictamen para este asunto específico.
              </p>
              <textarea
                value={dictamen}
                onChange={(e) => setDictamen(e.target.value)}
                className="w-full h-64 p-4 rounded-xl glass-input border border-slate-300/50 focus:ring-2 focus:ring-blue-500/50 outline-none font-serif text-lg leading-relaxed shadow-inner"
